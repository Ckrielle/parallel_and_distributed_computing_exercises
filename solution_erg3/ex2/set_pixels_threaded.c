...
#define RED_COEFFICIENT   100
#define GREEN_COEFFICIENT 100
#define BLUE_COEFFICIENT  100
...
void *set_pixels(void *ptr)
{
    ...
    for (int y = my_from; y < my_to; y++)
    {
        for (int x = 0; x < my_width; x++)
        {
            ...
            pixel->r = (pixel->r + RED_COEFFICIENT) % LIMIT;
            pixel->g = (pixel->g + GREEN_COEFFICIENT) % LIMIT;
            pixel->b = (pixel->b + BLUE_COEFFICIENT) % LIMIT;
        }
    }
...
int main()
{
    ...
    FILE *fOut = fopen("new_setpixels.bmp", "wb");
    if (!fIn || !fOut)
    {
        printf("File error.\n");
        return 0;
    ...
    for (int i = 0; i < NTHREADS; i++)
    {
        ...
        pthread_create(&threads[i], NULL, set_pixels, (void *) &thread_args[i]);
    }
